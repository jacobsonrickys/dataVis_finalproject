...
schools <- data.world::query(
  data.world::qry_sql("SELECT latitude, longitude, school_name FROM schools WHERE grades = 'Elementary' and school_type = 'public'"),
  dataset = project
  )
...
renderPlot({
  schools %>% ggplot(aes(longitude, latitude)) + geom_point()
})
