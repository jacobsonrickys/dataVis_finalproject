...
times <- data.world::query(
  data.world::qry_sql("SELECT zipcode, white_homeowners, market_time, school_time FROM apartments JOIN times using(place_id) JOIN (SELECT usa_zcta.zcta,
usa_zcta.b25003a_001/usa_zcta.b25003_001 as white_homeowners
FROM usa_zcta) as temp on temp.zcta = apartments.zipcode"),
  dataset = project
  )
times <- times %>% dplyr::filter(is.element(zipcode, zipcodes))
...
```{r echo = FALSE}
renderPlot({
  times %>% dplyr::select(-zipcode) %>% tidyr::gather(variable, value, -white_homeowners) %>% ggplot(aes(x = white_homeowners, y = value, color = variable)) + geom_point()
})
